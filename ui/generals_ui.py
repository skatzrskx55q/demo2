...
def render_general_phrases_html(phrases, best_phrase=None, matched_phrases=None):
    tiles_html = ""
...
def render(df):
    general_col = GENERAL_FILTER_COL
    all_generals = sorted(df[general_col].dropna().astype(str).unique())
    selected_general = st.selectbox(
        "Выберите General для просмотра (фильтр)",
        options=all_generals,
        index=None,
        placeholder="Выберите General",
    )

    if selected_general:
        rows = df[df[general_col] == selected_general].drop_duplicates(subset=["original_index"])
        for _, row in rows.iterrows():
            orig_idx = row["original_index"]
            all_examples = df.attrs["original_examples_map"].get(orig_idx, set())
            tiles_html = render_general_phrases_html(sorted(all_examples))
        ...
        try:
            ...
            if sem_results_raw:
                sem_groups = group_search_results(
                    ...
                    group_by_filter_cols=[GENERAL_FILTER_COL],
                )
                st.markdown("### 🧠 Результаты умного поиска")
                for group in sem_groups:
                    tiles_html = render_general_phrases_html(group["all_phrases"], best_phrase=group.get("best_phrase"))
            ...
            if exact_results_raw:
                ...
                for group in exact_groups:
                    tiles_html = render_general_phrases_html(group["all_phrases"], matched_phrases=group.get("matched_phrases"))
                    filters = group.get("filters", {})
                    general = filters.get(GENERAL_FILTER_COL, "")
                    block_html = f"""
                    <div style="border:1px solid #e0e0e0;border-radius:12px;padding:16px;margin-bottom:16px;background-color:#f9f9f9;color:#333;box-shadow:0 2px 6px rgba(0,0,0,0.05);">
